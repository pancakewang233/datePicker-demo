  ...
  inserted: function (el, binding, vnode) {
    ...
    const modelValue = function (value, len) {
      ...
      function replaceTime(value) {
        if (value.length > 5 && value.length < 9) {
          ...
          newValue = value.replace(
            ...
            "$1-$2-$3 $4:$5"
          ); // 格式化输入格式,2021-03-01 09:50
        } else if (value.length > 12) {
          newValue = value.replace(
            /^(\d{4})\D*(\d{1,2})\D*(\d{1,2})\D*(\d{1,2})\D*(\d{1,2})\D*(\d{1,2})\D*/,
            "$1-$2-$3 $4:$5:$6"
          ); // 格式化输入格式,2020-03-01 09:50:30
        } else {
          return false;
        }
      }
      ...
      if (temp.type === "date") {
        ...
        let key1 = drr[0];
        let key2 = drr[1];
        drr.length === 1 ? (_this[key1] = time) : (_this[key1][key2] = time);
      } else {
        if (!_obj) {
          // 处理简单的绑定
          ...
          if (objKey && objKey.length >= 2) {
            ...
            if (flag4) {
              _this[flag1][flag2][flag3][flag4][_obj.index][_obj.modelName] =
                len === 2 ? arr : time;
            } else if (flag3) {
              _this[flag1][flag2][flag3][_obj.index][_obj.modelName] =
                len === 2 ? arr : time;
            } else {
              _this[flag1][flag2][_obj.index][_obj.modelName] =
                len === 2 ? arr : time;
            }
          } else {
